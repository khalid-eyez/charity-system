...
class ngo extends charity_user
{
    ...
    public function newRequest($data,$files)
    {
      $this->data->get_request()->set_request($data);
      $reqid=$this->data->get_request()->save_request($this->data->get_id());
      if($reqid!=false){
      $res=false;
      $res=$this->uploadrequestattachments($files,$reqid);
...
if ($uploadOk == 0) {
 print "file not uploaded.";
  ...
  private function uploadrequestattachments($file,$reqid)
  {
...
$target_dir = "../../data/";
$target_file = $target_dir . basename($file['name'][$a]);
$filename=$file['name'][$a];
$tmpfile=$file["tmp_name"][$a];
  if(move_uploaded_file($tmpfile, $target_file)){
    array_push($uploade_files,$filename);
   }
  }
for($f=0;$f<count($uploade_files);$f++)
{
  $filenew=$uploade_files[$f];
  $sql="insert into request_images (imageURL,requestID) values ('$filenew',$reqid)";
  mysqli_query($this->data->conn,$sql);
